  ...
  $.fn.quizyMemoryGame = function(options) {
      ...
      // if it's the second click out of two (turning the second card)
        ...
        if(tClass == selItemClass){
          ...
          if(numMatches == itemsNum/2){
            ...
            if(opts.gameSummary){
			window.parent.globalScoreVariable = 100;
		...
		var $shuffleButton = $('#shuffleButton')	
          ...
          unbindClick($('div.'+itemsClass));
			$shuffleButton.attr('disabled', 'disabled').css('color','grey');    //remove shuffle click when wrong animation is displayed
          ...
          setTimeout( function(){
            ...
            $('.'+itemsClass).each(function(){
              ...
              if(correctItems.indexOf(myId) == -1){
                bindClick($(this));
				$shuffleButton.removeAttr('disabled').css('color','black');			//renable shuffle click 
    ...
    var showItem = function(el){
      ...
      switch(opts.animType){
        default:
        case 'fade':
          $('#'+topId).transition({opacity: 0},opts.animSpeed);
        break;
        case 'flip':
          el.flip({
        ...
        break;
        case 'scroll':
          $('#'+topId).transition({height: 'toggle', opacity:0.3},opts.animSpeed);
        break;
      }
    ...
    var hideItem = function(el){
      ...
      switch(opts.animType){
        default:
        case 'fade':
          $('#'+topId).delay(delayShow).transition({opacity: 1},opts.animSpeed);
        break;
        case 'flip':
          setTimeout( function(){
           el.revertFlip();
        ...
        case 'scroll':
          $('#'+topId).delay(delayShow).
                      transition({height: 'toggle', opacity:1},opts.animSpeed);
        break;
      }      
    }
    ...
    var showResIcon = function(type){
      if(opts.resultIcons){
        ...
        el.delay(time).transition({opacity: 1},time/2).delay(time/2).hide("explode", time/2);
      }
    }
